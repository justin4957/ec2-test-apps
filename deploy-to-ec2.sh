...
SOLID_CLIENT_ID=${SOLID_CLIENT_ID:-location-tracker-poc}
SOLID_CLIENT_SECRET=${SOLID_CLIENT_SECRET:-}
SOLID_REDIRECT_URI=${SOLID_REDIRECT_URI:-}
...
export GCP_SERVICE_ACCOUNT_JSON="GCP_SERVICE_ACCOUNT_JSON_VALUE"
export STRIPE_SECRET_KEY="STRIPE_SECRET_KEY_VALUE"
export STRIPE_PUBLISHABLE_KEY="STRIPE_PUBLISHABLE_KEY_VALUE"
export SOLID_ENABLED="SOLID_ENABLED_VALUE"
export SOLID_CLIENT_ID="SOLID_CLIENT_ID_VALUE"
export SOLID_CLIENT_SECRET="SOLID_CLIENT_SECRET_VALUE"
export SOLID_REDIRECT_URI="SOLID_REDIRECT_URI_VALUE"
DEPLOY_EOF
)
...
# Escape special characters in JSON for shell substitution
GCP_SERVICE_ACCOUNT_JSON_ESCAPED="${GCP_SERVICE_ACCOUNT_JSON//\\/\\\\}"
GCP_SERVICE_ACCOUNT_JSON_ESCAPED="${GCP_SERVICE_ACCOUNT_JSON_ESCAPED//\$/\\$}"
GCP_SERVICE_ACCOUNT_JSON_ESCAPED="${GCP_SERVICE_ACCOUNT_JSON_ESCAPED//\"/\\\"}"
DEPLOY_SCRIPT="${DEPLOY_SCRIPT//GCP_SERVICE_ACCOUNT_JSON_VALUE/$GCP_SERVICE_ACCOUNT_JSON_ESCAPED}"
...
DEPLOY_SCRIPT="${DEPLOY_SCRIPT//SOLID_ENABLED_VALUE/$SOLID_ENABLED}"
DEPLOY_SCRIPT="${DEPLOY_SCRIPT//SOLID_CLIENT_ID_VALUE/$SOLID_CLIENT_ID}"
DEPLOY_SCRIPT="${DEPLOY_SCRIPT//SOLID_CLIENT_SECRET_VALUE/$SOLID_CLIENT_SECRET}"
DEPLOY_SCRIPT="${DEPLOY_SCRIPT//SOLID_REDIRECT_URI_VALUE/$SOLID_REDIRECT_URI}"
...
bash << 'INNER_EOF'
    ...
    if [ ! -z "$SOLID_ENABLED" ]; then
        echo "🌐 Enabling Solid Pod authentication (Beta)"
        TRACKER_CMD="$TRACKER_CMD -e SOLID_ENABLED=${SOLID_ENABLED}"
        if [ ! -z "$SOLID_CLIENT_ID" ]; then
            TRACKER_CMD="$TRACKER_CMD -e SOLID_CLIENT_ID=${SOLID_CLIENT_ID}"
        fi
        if [ ! -z "$SOLID_REDIRECT_URI" ]; then
            TRACKER_CMD="$TRACKER_CMD -e SOLID_REDIRECT_URI=${SOLID_REDIRECT_URI}"
        fi
    fi
    ...
    if [ ! -z "$GEMINI_API_KEY" ] && [ ! -z "$GCP_SERVICE_ACCOUNT_JSON" ]; then
        ...
        DOCKER_CMD="$DOCKER_CMD -e GCP_SERVICE_ACCOUNT_JSON=${GCP_SERVICE_ACCOUNT_JSON}"
    else
        echo "⚠️  No Gemini API key or GCP service account provided, memes will not be generated"
        echo "      Required: GEMINI_API_KEY and GCP_SERVICE_ACCOUNT_JSON in .env.ec2"
    fi
